...
public class ConfigReader {
	...
	protected void readAll() throws IOException, ConfigFormatException {
	...
	private boolean isValid(String line, int cur) throws ConfigFormatException {
		if (!(line.isEmpty()||line.startsWith("#")||line.startsWith(ConfigDataTypes.BOOLEAN.getPrefix())||line.startsWith(ConfigDataTypes.BOOLEAN_ARRAY.getPrefix())||line.startsWith(ConfigDataTypes.DOUBLE.getPrefix())||line.startsWith(ConfigDataTypes.DOUBLE_ARRAY.getPrefix())||line.startsWith(ConfigDataTypes.INT.getPrefix())||line.startsWith(ConfigDataTypes.INT_ARRAY.getPrefix())||line.startsWith(ConfigDataTypes.STRING.getPrefix())||line.startsWith(ConfigDataTypes.STRING_ARRAY.getPrefix()))) {
			throw new ConfigFormatException("Config File is formated incorrectly: Line "+cur+" is not valid");
		}
		if (line.startsWith(ConfigDataTypes.STRING.getPrefix())) {
			if (!Pattern.compile("^S{1}:{1}[a-zA-Z]+={1}.+?;{1}$").matcher(line).matches()){
				throw new ConfigFormatException("Config File is formated incorrectly: String on line "+cur+" is not valid");
			}
		}else if (line.startsWith(ConfigDataTypes.INT.getPrefix())) {
			if (!Pattern.compile("^I{1}:{1}[a-zA-Z]+={1}-{0,1}[0-9]+;{1}$").matcher(line).matches()) {
				throw new ConfigFormatException("Config File is formated incorrectly: Integer on line "+cur+" is not valid");
			}else if (Long.parseLong(line.replaceAll("^I{1}:{1}[A-Za-z]+={1}", "").trim().replaceAll(";$", ""))>Integer.MAX_VALUE) {
				throw new ConfigFormatException("Config File is formated incorrectly: Integer on line "+cur+" is too large");
			}else if (Long.parseLong(line.replaceAll("^I{1}:{1}[A-Za-z]+={1}", "").trim().replaceAll(";$", ""))<Integer.MIN_VALUE) {
				throw  new ConfigFormatException("Config File is formated incorrectly: Integer on line "+cur+" is too small");
			}
			...
			if (!Pattern.compile("^B{1}:{1}[a-zA-Z]+={1}(true|false);{1}$").matcher(line).matches()) {
				throw  new ConfigFormatException("Config File is formated incorrectly: Boolean on line "+cur+" is not valid");
			}
			...
			if (!Pattern.compile("^D{1}:{1}[a-zA-Z]+={1}-{0,1}[0-9]+\\.{1}[0-9]+;{1}$").matcher(line).matches()) {
				throw new ConfigFormatException("Config File is formated incorrectly: Double on line "+cur+" is not valid");
			}else if (Double.parseDouble(line.replaceAll("^D{1}:{1}[a-zA-Z]+={1}", "").trim().replaceAll(";$", ""))>Double.MAX_VALUE) {
				throw new ConfigFormatException("Config File is formated incorrectly: Double on line "+cur+" is too large");
			}else if (Double.parseDouble(line.replaceAll("^D{1}:{1}[a-zA-Z]+={1}", "").trim().replaceAll(";$", ""))<-Double.MAX_VALUE) {
				throw new ConfigFormatException("Config File is formated incorrectly: Double on line "+cur+" is too small");
			}
			...
			if (!Pattern.compile("^A{1}\\[{1}S{1}\\]{1}:{1}[A-Za-z]+={1}<{1}(.+,?)+>{1};{1}$").matcher(line).matches()) {
				throw new ConfigFormatException("Config File is formated incorrectly: String Array on line "+cur+" is not valid");
			}
			...
			if (!Pattern.compile("^A{1}\\[{1}I{1}\\]{1}:{1}[A-Za-z]+={1}<{1}(-{0,1}[0-9]+,?)+>{1};{1}$").matcher(line).matches()) {
				throw new ConfigFormatException("Config File is formated incorrectly: Integer array on line "+cur+" is not valid");
			} else {  
				...
				for (int k=0;k<i.length;k++) {
					if (Long.parseLong(i[k])>Integer.MAX_VALUE||Long.parseLong(i[k])<Integer.MIN_VALUE) {
						throw new ConfigFormatException("Config File is formated incorrectly: Integer in array on line "+cur+" is too large or small");
					}
				}
			}
			...
			if (!Pattern.compile("^A{1}\\[{1}B{1}\\]{1}:{1}[A-Za-z]+={1}<{1}((true|false),?)+>{1};{1}$").matcher(line).matches()) {
				throw new ConfigFormatException("Config File is formated incorrectly: Boolean array on line "+cur+" is not valid");
			}
			...
			if (!Pattern.compile("^A{1}\\[{1}D{1}\\]{1}:{1}[A-Za-z]+={1}<{1}(-{0,1}[0-9]+\\.{1}[0-9]+,?)+>{1};{1}$").matcher(line).matches()) {
				throw new ConfigFormatException("Config File is formated incorrectly: Double array on line "+cur+" is not valid");
			} else {
				...
				for (int k=0;k<i.length;k++) {
					if (Double.parseDouble(i[k])>Double.MAX_VALUE||Double.parseDouble(i[k])<-Double.MAX_VALUE) {
						throw new ConfigFormatException("Config File is formated incorrectly: Double in array on line "+cur+" is too large or small");
					}
				}
			}
		}
